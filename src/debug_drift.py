...
def prepare_data(df: pd.DataFrame, num_features: list, cat_features: list):
     """
     ...
     return  -> pd.DataFrame
     """
     filtered_data = df.loc[(df.lpep_pickup_datetime >= datetime.datetime(2022, 2, 2, 0, 0)) &
...
def create_and_run_test_suite(ref_data, curr_data, column_mapping, save_file=False):
     """
...
def create_and_run_report(reference_data, current_data, column_mapping, save_file=True):
     """
