...
def prep_db():
     ...
     # Use 'database' instead of 'localhost' because it is not an stand alone
     # with psycopg.connect("host=localhost port=5432 user=postgres password=example", autocommit=True) as conn:
     with psycopg.connect("host=database port=5432 user=postgres password=example", autocommit=True) as conn:
          # Check if the 'test' database exists, create it if not
          res = conn.execute("SELECT 1 FROM pg_database WHERE datname='test'")
          if len(res.fetchall()) == 0:
               conn.execute("create database test;")
          # Connect to the 'test' database
          with psycopg.connect("host=database port=5432 dbname=test user=postgres password=example") as conn:
...
def main():
     ...
     # Connect to the 'test' database
     with psycopg.connect("host=database port=5432 dbname=test user=postgres password=example", autocommit=True) as conn:
